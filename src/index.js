...
async function runMigrations() {
  ...
  await db.query(`CREATE TABLE IF NOT EXISTS replies (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ...
  await db.query(`CREATE INDEX IF NOT EXISTS idx_reviews_location ON reviews(location_id)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_reviews_date ON reviews(review_date DESC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_replies_status ON replies(status)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_replies_token ON replies(approval_token)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_locations_user ON locations(user_id)`);
